...
class CategoryManagerTest extends TestCase
{
    protected function createEntityManagerMock($fail = false)
    {
        ...
        $entityManager->method('getRepository')
            ->with($this->stringContains('AppBundle\Entity\Category'))
            ->will($this->returnCallback(function () use ($fail) {
                if ($fail) {
    ...
    public function testCreatePositive($entity, $result)
    {
        ...
        $manager = new CategoryManager($entityManagerMock);
    ...
    {
        $manager = new CategoryManager($this->createEntityManagerMock());

        $this->assertInstanceOf(CategoryRepository::class, $manager->getRepository());
    }

    public function testGetRepositoryNegative()
    {
        $manager = new CategoryManager($this->createEntityManagerMock(true));

        $this->assertNotInstanceOf(CategoryRepository::class, $manager->getRepository());
    }
